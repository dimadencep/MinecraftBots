...
function run(client) {
    client._client.on('map', ({ data }) => {
        ...
        image.writeImage(`../assets/`+client._client.username+'.png', function (err) {
            if (err) {
				console.log("Ошибка записи капчи!")
			};
        });

        tesseract.process(`../assets/`+client._client.username+'.png', function(err, text) {
            if(err) {
                console.error(err);
            } else {
                console.log(text);
